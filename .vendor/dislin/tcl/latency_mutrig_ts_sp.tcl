...
set fd0 [open "/home/yifeng/packages/online_dpv2/online/fe_board/fe_scifi/records/ingress_mask03_randInj/random.txt"]
...
# random injection file
set fd0 [open "/home/yifeng/packages/online_dpv2/online/fe_board/fe_scifi/records/ingress_mask03_varHdrDelay/delay850.txt"]
set lines [split [read $fd0] "\n"]
close $fd0;
#puts [llength $lines]
set i_line 0
...
Dislin::setpag da4p
Dislin::metafl cons
Dislin::setfil "plot_packet_latency_ingress_sp.pdf"
Dislin::disini 
Dislin::pagera 
Dislin::complx 
#Dislin::ticks  1 x
...
for {set i 1} {$i <= 2} {incr i} {
    if {$i == 1} {
        set legendName "random injection"
    }
    if {$i == 2} {
        set legendName "header delay \(ns\) = 850"
    }
...
for {set i 1} {$i <= 2} {incr i} {
    ...
    if {$i == 1} {
        Dislin::graf 000 2000 0.0 200 0.0 1e5 0.0 0.25e5
    }
    if {$i == 2} {
