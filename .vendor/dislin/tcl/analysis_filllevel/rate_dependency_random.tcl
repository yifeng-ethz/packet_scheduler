...
#
#                   study:          dependency on rate
#                   
#                   condition:      random injection
...
for {set i 1} {[expr $i < $nFile+1]} {incr i} {
    # 0.1) read file 
    set int [expr $int / 2]
    set fd0 [open "/home/yifeng/packages/online_dpv2/online/fe_board/fe_scifi/records/filllevel_ringcam/cam-0_inj-rdm_interval-${int}_delay-100.txt"]
    ...
    foreach line $lines {
        ...
        # for this file process the y points
            ...
            set points_sum 0
            foreach point $line {
                incr points_sum $point
            }
            
            # scale or log each y point
            foreach point $line {
                # normalize
                #set point [expr 1.0*$point / $points_sum]
                if {$point == 0} {
                    set point_tmp 0
                } else {
...
# 1) config 
set subplot_y_length 400
Dislin::scrmod revers
Dislin::setpag da4p
Dislin::metafl cons
Dislin::setfil "./plot/plot_filllevel_rate_cam-0_random.pdf"
Dislin::disini 
Dislin::pagera 
Dislin::complx 
#Dislin::ticks  1 x
...
Dislin::legtit "injection interval (inj=random, cam=0)"
set color255 1
set int 2000
for {set i 1} {$i < $nFile+1} {incr i} {
    set dly [expr $i*100]
    set int [expr $int / 2]
    Dislin::leglin $cbuf "interval = $int" $i
...
for {set i 1} {$i < $nFile+1} {incr i} {
    ...
    Dislin::axspos 300 [expr 2200-$subplot_y_length*[expr $i-1]]
    
    Dislin::graf 0 1024 0 128 0 $max_y 0.0 $tick_y
