...
for {set i 1} {[expr $i < $nFile+1]} {incr i} {
    # 0.1) read file 
    set cam_id [expr $i - 1]
    set fd0 [open "/home/yifeng/packages/online_dpv2/online/fe_board/fe_scifi/records/filllevel_ringcam/cam-${cam_id}_inj-hdr_rate-1_delay-100.txt"]
...
Dislin::setpag da4p
Dislin::metafl cons
Dislin::setfil "./plot/plot_filllevel_cam_balance_rate-1.pdf"
Dislin::disini 
Dislin::pagera 
Dislin::complx 
#Dislin::ticks  1 x
...
Dislin::legtit "CAM index (rate=1 delay=100)"
set color255 1
for {set i 1} {$i < $nFile+1} {incr i} {
    set dly [expr $i*100]
    Dislin::leglin $cbuf "index = ${dly}" $i
