# DV_INT Long-Run Plan: FEB datapath sanity matrix
...
## 3. Required harness delta before running the matrix

These are harness tasks, not datapath-model changes.

1. Add a `tb_int_emut_cfg` object to `uvm/tb_int_pkg.sv` with one profile per datapath / emulated ASIC.
2. Add a build/pre-run Avalon/MM programming path in `tb_int` for `emulator_mutrig` only.
3. Keep `frame_rcv_ip`, `mts_processor`, `ring_buffer_cam`, and `feb_frame_assembly` on default CSR unless an explicit debug case later proves otherwise.
4. Add one long-run base test, for example `tb_int_longrun_sanity_test`, that:
   - loads one case profile by ID
   - programs emulator CSR after reset
   - runs the normal run-control sequence
   - emits one per-case summary row
5. Add a launcher script under `tb_int/scripts/` that:
   - compiles once
   - fans out 16 runs in parallel
   - writes each run into its own `sim_runs/<case_id>/`
   - merges a compact regression summary at the end

The plan assumes **no** `.ipx` work and **no** Qsys regeneration.
...
1. Add emulator build/pre-run CSR programming support to `tb_int`.
2. Add one generated-profile long-run test and one case launcher.
3. Implement and debug the 16-case `B0` bucket first.
4. Expand to `B4` to cover short mode.
5. Expand to `B1/B5` and `B2/B6`.
6. Add `B3/B7` last because cluster-domain cases are the most correlated and least forgiving.

This keeps the first closure step on the simplest, most interpretable long runs.
