...
REPORT_ROOT="${REPORT_ROOT:-${TB_DIR}/REPORT/math}"
MODEL_DIR="${MODEL_DIR:-${REPORT_ROOT}/queueing_model}"
PLOT_DIR="${PLOT_DIR:-${REPORT_ROOT}/plots/analytical}"
...
gcc -O2 -Wall -Wextra -std=c11 \
  -I"${DISLIN_DIR}" \
  "${IP_DIR}/tb_old_reference/scripts/opq_vs_time_merger_contour_plot.c" \
  -L"${DISLIN_DIR}" \
  -Wl,-rpath,"${DISLIN_DIR}" \
  -ldislin -lm \
