...
static void render_plot(const ratio_grid_t *ratio_grid, const char *output_path) {
  ...
  axspos(420, 1750);
  axslen(1950, 1100);
  graf(xmin, xmax, -0.2f, 0.2f, ymin, ymax, 50.0f, 10.0f);
  ...
  height(16);
  txtjus("RIGHT");
  messag("1e6x", ztick_label_x, (int) floorf(yposn(ymax) + 0.5f) - 8);
  messag("1e5x", ztick_label_x, (int) floorf(yposn(ymin + (5.0f / 6.0f) * (ymax - ymin)) + 0.5f) - 8);
  messag("1e4x", ztick_label_x, (int) floorf(yposn(ymin + (4.0f / 6.0f) * (ymax - ymin)) + 0.5f) - 8);
  messag("1e3x", ztick_label_x, (int) floorf(yposn(ymin + (3.0f / 6.0f) * (ymax - ymin)) + 0.5f) - 8);
  messag("100x", ztick_label_x, (int) floorf(yposn(ymin + (2.0f / 6.0f) * (ymax - ymin)) + 0.5f) - 8);
  messag("10x", ztick_label_x, (int) floorf(yposn(ymin + (1.0f / 6.0f) * (ymax - ymin)) + 0.5f) - 8);
  messag("1x", ztick_label_x, (int) floorf(yposn(ymin) + 0.5f) - 8);
  txtjus("CENT");
  height(30);
  angle(270);
  messag("time-merger loss / OPQ loss", ztitle_x,
         ((int) floorf(yposn(ymax) + 0.5f) + (int) floorf(yposn(ymin) + 0.5f)) / 2);
  angle(0);
  txtjus("LEFT");
