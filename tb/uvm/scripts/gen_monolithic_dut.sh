...
PKT_DIR="$(cd "${TB_DIR}/../.." && pwd)"
OUT_DIR="${1:-${PKT_DIR}/tb/rtl_gen_monolithic}"
...
IMPL_VHD="${OUT_DIR}/ordered_priority_queue_dut_impl.vhd"
WRAP_VHD="${OUT_DIR}/ordered_priority_queue_dut.vhd"
TCL_FILE="${OUT_DIR}/gen_monolithic_dut.tcl"
...
set params(output_name) "ordered_priority_queue_dut_impl"
...
entity ordered_priority_queue_dut is
    port (
...
end entity ordered_priority_queue_dut;

architecture rtl of ordered_priority_queue_dut is
begin
    u_impl : entity work.ordered_priority_queue_dut_impl
        generic map (
            N_LANE              => 2,
            MODE                => "MERGING",
            TRACK_HEADER        => true,
