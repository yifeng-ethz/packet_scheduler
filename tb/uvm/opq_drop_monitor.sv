    ...
    forever begin
      ...
      for (int lane = 0; lane < OPQ_N_LANE; lane++) begin
        if (vif.valid[lane] === 1'b1) begin
          ...
          item.exact_pre_valid = vif.exact_pre_valid[lane];
          item.exact_pre_ts = vif.exact_pre_ts[lane];
          item.exact_pre_serial = vif.exact_pre_serial[lane];
          item.exact_pre_shd_cnt = vif.exact_pre_shd_cnt[lane];
          item.exact_pre_hit_cnt = vif.exact_pre_hit_cnt[lane];
          item.exact_post_valid = vif.exact_post_valid[lane];
          item.exact_post_ts = vif.exact_post_ts[lane];
          item.exact_post_serial = vif.exact_post_serial[lane];
