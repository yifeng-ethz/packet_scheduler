...
| Allocator and DRR transition space | stmt=93.18, branch=91.25, cond=78.75, fsm_trans=58.82, toggle=74.88 (min across 2 instances) | real gap: The remaining low FSM-transition coverage lines up with the still-open bursty DRR random probe plus the lack of signed-off 4-lane DV and Arria 10 synthesis evidence. | BUG-009-R; opq_cross_drr_bursty_random_test; opq_prof_missing_empty_frame_test @ OPQ_N_LANE=4; doc/SIGNOFF.md standalone_syn | Refresh the bursty DRR random evidence and record real 4-lane DV plus A10 standalone closure before expanding the signoff claim. |
