...
// Version : 26.3.14
// Date    : 20260417
// Change  : Implement a VHDL-compatible native SV CSR plane with programmable DRR allowance and live counter readback
//------------------------------------------------------------------------------
...
`ifdef OPQ_USE_NATIVE_SV
  ...
  localparam int unsigned TICKET_FIFO_DEPTH_CONST = 256;
  ...
  localparam int unsigned PAGE_RAM_DEPTH_CONST = 65536;
  ...
  localparam int unsigned VERSION_BUILD_CONST = 417;
  localparam logic [31:0] VERSION_DATE_CONST = 32'd20260417;
  localparam logic [31:0] VERSION_GIT_CONST = 32'h0893_20D0;
  ...
  always_ff @(posedge d_clk) begin : proc_native_csr
    ...
    avs_csr_readdatavalid <= avs_csr_read || csr_read_d;
    csr_read_d <= avs_csr_read;
    if (avs_csr_read) begin
      avs_csr_readdata <= csr_decode_word(avs_csr_address);
    end
    ...
    if (d_reset) begin
      ...
      if (clear_counters_v) begin
        ...
        for (int lane = 0; lane < OPQ_N_LANE_LOCAL; lane++) begin
          if (asi_ingress_valid_bus[lane] && !csr_lane_mask[lane]) begin
          ...
          if (native_handle_we_dbg[lane] && native_handle_flag_dbg[lane]) begin
            csr_drop_shd_cnt[lane] <= sat_add32(csr_drop_shd_cnt[lane], 32'd1);
            csr_drop_hit_cnt[lane] <= sat_add32(
        ...
        if (aso_egress_valid && aso_egress_ready) begin
          ...
          if (csr_ft_rd_in_packet_v) begin
            if (csr_ft_rd_header_idx_v < HDR_SIZE_CONST[2:0]) begin
              ...
              csr_ft_rd_hits_pending_v = aso_egress_data[15:8];
